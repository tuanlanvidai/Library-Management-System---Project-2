...
public class TrangChuDAO {
    ...
    public List<OverdueBook> getListOverDueReaders() {
        ...
        SELECT 
            ...
            (DATEDIFF(CURRENT_DATE, bb.dueDate) * s.lateFeePerDay) AS fineMoney
        FROM BorrowBook bb
        CROSS JOIN Setting s -- Lấy cấu hình từ bảng Setting
        WHERE bb.isDeleted = 0 
          AND DATEDIFF(CURRENT_DATE, bb.dueDate) > 0;
    """;

        try (Connection connection = DriverManager.getConnection(ConfigUtils.dbConnect, ConfigUtils.username, ConfigUtils.password); PreparedStatement statement = connection.prepareStatement(query); ResultSet resultSet = statement.executeQuery()) {
        ...
        return overdueBooks; // Trả về danh sách sách quá hạn
    }
